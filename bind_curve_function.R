...
bind_curve <- function(file) {
  ...
  x_axis_label <- "Thrombin Conc(nM)"
  y_axis_label <- bquote("("~I[0]-I~") /"~I[0]~"")
  ...
  Bmax_string <- bquote(B[max]~"="~.(round(B_max, 3))~""%+-%""~.(round(B_max_error, 3)))
  Ka_string <- bquote(K[a]~"="~.(round(K_a, 3))~""%+-%""~.(round(K_a_error, 3)))
